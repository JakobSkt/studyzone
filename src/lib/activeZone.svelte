...
<main>
    ...
    <div class:musicActive={music} class="fixed right-0 inset-y-0 my-auto bg-zinc-900 rounded-xl" id="musicPanel">
        {#if music}
            ...
            {:else}
                <svg on:click={() => music = !music} class="absolute left-0 inset-y-0 my-auto w-12 h-12 stroke-white cursor-pointer -ml-12 bg-zinc-900 rounded-full p-2 hover:bg-white hover:stroke-zinc-900 transition duration-200" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" data-slot="icon">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M15.75 19.5 8.25 12l7.5-7.5" />
                </svg>
        {/if}
        <iframe class="py-8 px-8" style="border-radius:12px" src={musicUrl} width="100%" height="90%" frameBorder="0" allowfullscreen="no" allow="autoplay; clipboard-write; encrypted-media; fullscreen; picture-in-picture" loading="lazy"></iframe>
    </div>
